...
This report shows the test cases developed through \textit{JWebUnit} for testing the \textit{true} vulnerabilities described in the \textit{Assignment 7}. Finally, the last section is dedicated to the fix of such vulnerabilities.
\bigskip
\subsection{Package}
The package contains:
\begin{itemize}
	\item the report
	\item \textit{fixed} folder, containing the modified files to 
	\item \textit{notfixed} folder, containing the original versions of the files
	\item \textit{dumps} folder, containing the dumps from the database
	\item \textit{src} folder, source code for the JWebUnit test cases
\end{itemize}
\end{homeworkProblem}
\clearpage
