...
\\Similar Vulnerabilities\footnote{their test cases are based on the ones of this vulnerability}: \\16,18,19,63,70,71,87,88,89,90,93,126,138,141,165,180,181,186,194,200,201,230,238
	\begin{table}[h]
		\begin{center}
    		\begin{tabular}{ | c | c |}
    			\hline
...
\\Similar Vulnerabilities\footnote{their test cases are based on the ones of this vulnerability}: 90,126,138,207
	\begin{table}[h]
		\begin{center}
    		\begin{tabular}{ | c | c | c |}
    			\hline
...
\\Similar Vulnerabilities\footnote{their test cases are based on the ones of this vulnerability}: 147,148,257
	\begin{table}[h]
		\begin{center}
    		\begin{tabular}{ | c | c | }
    			\hline
...
\begin{center}
	File: ViewAssignments.php
\\Similar Vulnerabilities\footnote{their test cases are based on the ones of this vulnerability}: 184
	\begin{table}[h]
		\begin{center}
    		\begin{tabular}{ | c | c | }
    			\hline
...
\end{center}
\subsection{JWebUnit test cases}
\subsubsection{page and page2, selectclass}
The code is adapted from the one of \textit{Vulnerability 11} at page ~\pageref{sec:V11}.
\subsubsection{onpage}
\textbf{VERIFICARE}
\begin{lstlisting}[language=Java,caption=jwebunit test code for \textit{onpage}]
...
\begin{lstlisting}[language=Java,caption=cleanup function]
	public void cleanup(){
		...
		tester.checkCheckbox("delete[]",myCheckbox.getAttribute("value"));
		Functions.click(tester,"Edit",1);
		tester.setTextField("title","first");
		Functions.click(tester,"Edit Semester",1);
		Functions.click(tester,"Log Out",0);
		tester = null;
	}
